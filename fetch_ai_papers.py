...
Retrieve at least 20 recent (last 5 years) scientific articles about 
"artificial intelligence" applied to "software development" or "software engineering" 
from public APIs:
- Semantic Scholar
- OpenAlex
- CrossRef
...
Features:
- Query: "artificial intelligence" AND ("software development" OR "software engineering")
...
OUTPUT_CSV = os.path.join(DATA_DIR, "papers.csv")
...
DEFAULT_HEADERS = {
    "User-Agent": "fetch_ai_papers/2.0 (Educational Research - AI-focused)"
}
...
# Improved Query - stricter focus on AI applied to software development
# Includes: AI, Machine Learning, Deep Learning, Generative AI, Neural Networks
...
# Enterprise / empirical focus keywords (prefer these to ensure industry/enterprise relevance)
ENTERPRISE_KEYWORDS = {
    "enterprise", "industry", "industrial", "company", "companies", "business", "organization", "organización",
    "industrial application", "industrial setting", "industrial setting", "production", "deployment", "deploy",
    "case study", "case studies", "use case", "use cases", "real-world", "real world", "empirical", "evaluation",
    "industry 4.0", "enterprise architecture", "industrial", "industrial application", "enterprise application",
}

# Development related keywords (reinforce software development context)
DEVELOPMENT_KEYWORDS = {
    "software development", "software engineering", "development", "devops", "sdlc", "ci/cd", "deployment",
    "software architecture", "code", "automation testing", "testing", "software testing", "build", "deployment"
}
...
def make_paper_record(title: str, authors: List[str], year: Optional[int], 
                     ...
                     open_access: bool, source: str, enterprise_relevant: bool=False) -> Dict:
    """
    ...
    return {
        ...
        "source": source,
        "enterprise_relevant": bool(enterprise_relevant),
...
def is_relevant_article(record: Dict) -> Dict[str, bool]:
    ...
    for exclude_kw in EXCLUSION_KEYWORDS:
        if exclude_kw.lower() in text_to_check:
            return {"ai": False, "development": False, "enterprise": False, "empirical": False, "relevant": False}

    # AI present?
    ai_found = any(ai_kw.lower() in text_to_check for ai_kw in AI_KEYWORDS)

    # Development present?
    dev_found = any(dev_kw.lower() in text_to_check for dev_kw in DEVELOPMENT_KEYWORDS)

    # Enterprise or empirical presence
    enterprise_found = any(ent.lower() in text_to_check for ent in ENTERPRISE_KEYWORDS)
    empirical_found = any(ent.lower() in text_to_check for ent in {"case study", "case studies", "empirical", "real world", "real-world", "industrial application", "industry", "enterprise", "evaluation"})

    # Relevant if AI + Development + (enterprise OR empirical)
    is_relevant = ai_found and dev_found and (enterprise_found or empirical_found)
    return {"ai": ai_found, "development": dev_found, "enterprise": enterprise_found, "empirical": empirical_found, "relevant": is_relevant}
    ...
    for attempt in range(3):
        ...
        for item in items:
            try:
                ...
                # Validate relevance before adding (AI + Development + (enterprise|empirical))
                relevance_flags = is_relevant_article(record)
                if relevance_flags.get("relevant"):
                    # annotate record with enterprise relevance
                    record["enterprise_relevant"] = bool(relevance_flags.get("enterprise") or relevance_flags.get("empirical"))
...
def save_csv(records: List[Dict], filepath: str):
    ...
    fieldnames = ["title", "authors", "year", "doi", "url", "abstract", "open_access", "enterprise_relevant", "source", "retrieved_at"]
    ...
    with open(filepath, "w", encoding="utf-8", newline="") as f:
        ...
        for record in records:
            ...
            # ensure enterprise_relevant field exists
            if "enterprise_relevant" not in row:
                row["enterprise_relevant"] = False
            writer.writerow(row)
...
def print_summary(records: List[Dict]):
    ...
    enterprise_count = sum(1 for r in records if r.get("enterprise_relevant"))
    
    print(f"Summary Statistics:")
    print(f"  Total Papers: {len(records)}")
    print(f"  Open Access: {oa_count}")
    print(f"  Non-OA: {non_oa_count}")
    print(f"  Enterprise/Industrial/Empirical: {enterprise_count}")
    print(f"  Query: {QUERY}\n")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch AI-applied software dev papers from public APIs"
    )
